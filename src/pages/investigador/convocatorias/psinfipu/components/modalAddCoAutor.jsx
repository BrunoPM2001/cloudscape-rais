...
export default ({ close, reload, id }) => {
  ...
  const { loading, options, setOptions, value, setValue, setAvoidSelect } =
    useAutosuggest("investigador/convocatorias/psinfipu/listadoGrupoMiembro");
  ...
  const agregarIntegrante = async () => {
    if (validateForm()) {
      ...
      const res = await axiosBase.postForm(
        "investigador/convocatorias/psinfipu/agregarIntegrante",
        { ...form, id, ...formValues }
      );
      const data = res.data;
      ...
      <SpaceBetween size="m">
        <Alert header="Acerca de este integrante">
          <>
            ...
            <strong>Coordinador, Adjunto, Adherente o Títular</strong>
          </>
        </Alert>
